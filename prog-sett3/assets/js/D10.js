...
me.skills = ['HTML', 'CSS'];
console.log('Array integrato:', me);
...
console.log('Array skills integrato:', me);
...
me.skills.pop();
console.log('Array skills ridotto:', me);
...
console.log(deleteOne('prapapippo', false));
...
function onlyLetters(str) {
  stringOnly = str.replace(/\d/g, '');
  return stringOnly;
}

console.log(onlyLetters('ho 3 cani e 4 gatti'));
...
console.log(isThisAnEmail('nicolefilippi01@gmail.com'));

/* ESERCIZIO 7
  Scrivi una funzione chiamata "whatDayIsIt" che ritorna il giorno della settimana corrente.
*/
function whatDayIsIt(myDay) {
...
console.log('Somma:', risultato.somma);
console.log('Valori:', risultato.values);
...
function howManyDays(data) {
  ...
  let result = Math.floor(differenza / (1000 * 60 * 60 *24));
  return result;
}
...
let myBirthday = new Date('2023-11-30');
let res = isTodayMyBirthday(myBirthday);
...
let result = deleteProp(sayan, 'surname');
console.log(result);
...
function onlyTheYears() {
  let yearProp = movies.map((film) => film.Year);
  return yearProp;
}
...
function onlyInLastMillennium() {
  return lastMillennium = movies.filter((film) => film.Year < 2000);
}
...
const sumAllTheYears = (film) => {
  return film.reduce((val, el) => val + parseInt(el.Year), 0);
}
...
function searchByTitle(movies, str) {
  let movieMatch = movies.filter ((film) => {
    return film.Title.toLowerCase().includes(str.toLowerCase());
  });
  return movieMatch;
}
let movieMatch = searchByTitle(movies, 'avengers')
...
let searchString = searchAndDivide(movies, 'lord');
...
myUl.appendChild(nuovoElemento);
}
